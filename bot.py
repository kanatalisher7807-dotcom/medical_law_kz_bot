import os
import json
import logging
from typing import Optional, Dict, Any, List

from aiogram import Bot, Dispatcher, types
from aiogram.utils import executor
from aiogram.types import ReplyKeyboardMarkup, KeyboardButton

logging.basicConfig(level=logging.INFO)

TOKEN = os.getenv("TELEGRAM_TOKEN")
FAQ_PATH = os.getenv("FAQ_PATH", "faq.json")

DISCLAIMER = (
    "⚠️ Ответ носит информационный характер и не является официальным юридическим заключением. "
    "Для индивидуальной ситуации используйте кнопку «✉️ Задать вопрос преподавателю»."
)

SECTIONS = [SECTION_TO_QUERY = {
    "⚖️ Медицинские ошибки": "медицинская ошибка",
    "🚨 Инциденты": "инцидент",
    "🏥 Жалобы пациента": "жалоба",
    "✍️ Информированное согласие": "информированное согласие",
    "🔒 Врачебная тайна": "врачебная тайна",
    "👮 Ответственность медработников": "ответственность медработников",
}

    "⚖️ Медицинские ошибки",
    "🚨 Инциденты",
    "🏥 Жалобы пациента",
    "✍️ Информированное согласие",
    "🔒 Врачебная тайна",
    "👮 Ответственность медработников",
    "📄 Нормативная база",
    "✉️ Задать вопрос преподавателю",
    "🧪 Мини-тесты",
]

menu = ReplyKeyboardMarkup(resize_keyboard=True)
menu.add(KeyboardButton(SECTIONS[0]), KeyboardButton(SECTIONS[1]))
menu.add(KeyboardButton(SECTIONS[2]), KeyboardButton(SECTIONS[3]))
menu.add(KeyboardButton(SECTIONS[4]), KeyboardButton(SECTIONS[5]))
menu.add(KeyboardButton(SECTIONS[6]))
menu.add(KeyboardButton(SECTIONS[7]), KeyboardButton(SECTIONS[8]))

def load_faq() -> List[Dict[str, Any]]:
    try:
        with open(FAQ_PATH, "r", encoding="utf-8") as f:
            data = json.load(f)
        if not isinstance(data, list):
            return []
        return data
    except Exception as e:
        logging.exception("Failed to load FAQ: %s", e)
        return []

FAQ = load_faq()

def find_answer(user_text: str) -> Optional[Dict[str, Any]]:
    text = (user_text or "").lower()
    best = None
    best_score = 0

    for entry in FAQ:
        keywords = entry.get("keywords") or []
        score = 0
        for kw in keywords:
            if isinstance(kw, str) and kw.lower() in text:
                score += 1
        if score > best_score:
            best_score = score
            best = entry

    return best if best_score > 0 else None

if not TOKEN:
    raise RuntimeError("TELEGRAM_TOKEN is not set.")

bot = Bot(token=TOKEN)
dp = Dispatcher(bot)

@dp.message_handler(commands=["start"])
async def start(message: types.Message):
    text = (
        "Здравствуйте! Я бот кафедры медицинского права.\n\n"
        "Я помогаю с типовыми вопросами: медицинские ошибки, инциденты, жалобы, "
        "информированное согласие, врачебная тайна, ответственность.\n\n"
        "Выберите раздел кнопками ниже или просто напишите вопрос текстом."
    )
    await message.answer(text, reply_markup=menu)

@dp.message_handler(commands=["help", "menu"])
async def help_cmd(message: types.Message):
    await message.answer("Выберите раздел кнопками или напишите вопрос текстом.", reply_markup=menu)

@dp.message_handler(lambda m: m.text == "📄 Нормативная база")
async def law_base(message: types.Message):
    text = (
        "📄 Нормативная база (ориентиры):\n"
        "• Кодекс РК «О здоровье народа и системе здравоохранения»\n"
        "• УК / КоАП / ГК / ТК РК — по ситуации\n"
        "• Внутренние регламенты медорганизации и приказы уполномоченного органа\n\n"
        "Если напишете тему (например, «врачебная тайна»), я подскажу типовой блок норм."
    )
    await message.answer(text, reply_markup=menu)

@dp.message_handler(lambda m: m.text == "✉️ Задать вопрос преподавателю")
async def ask_teacher(message: types.Message):
    await message.answer(
        "Напишите ваш вопрос одним сообщением.\n"
        "Формат: *Тема* → *Суть вопроса*.\n"
        "Не указывайте лишние персональные данные.",
        parse_mode="Markdown",
        reply_markup=menu,
    )

@dp.message_handler(lambda m: m.text == "🧪 Мини-тесты")
async def mini_tests(message: types.Message):
    await message.answer(
        "Мини‑тесты подключим на следующем шаге.\n"
        "Пока можете написать вопрос текстом — я отвечу по базе знаний.",
        reply_markup=menu,
    )
@dp.message_handler(lambda m: m.text == "⚖️ Медицинские ошибки")
async def btn_med_errors(message: types.Message):
    text = (
        "⚖️ Медицинские ошибки — раздел про типовые ситуации:\n"
        "• подозрение на ошибку/дефект оказания помощи\n"
        "• неправильный диагноз/назначение\n"
        "• осложнение после вмешательства (как отличать от ошибки)\n\n"
        "Напишите 1–2 слова (например: «неправильный диагноз», «ошибка назначения») "
        "или опишите ситуацию коротко — я подскажу типовой алгоритм фиксации и обращения."
    )
    await message.answer(text, reply_markup=menu)


@dp.message_handler(lambda m: m.text == "🚨 Инциденты")
async def btn_incidents(message: types.Message):
    text = (
        "🚨 Инциденты — раздел про конфликтные/нештатные ситуации в медорганизации:\n"
        "• отказ в приёме/госпитализации\n"
        "• грубость персонала, нарушение порядка оказания помощи\n"
        "• спорные действия/бездействие при обращении\n\n"
        "Напишите, что случилось (2–3 фразы): где, когда, кто, в чём проблема — "
        "я подскажу, как правильно зафиксировать факт и куда обращаться."
    )
    await message.answer(text, reply_markup=menu)


@dp.message_handler(lambda m: m.text == "🏥 Жалобы пациента")
async def btn_patient_complaints(message: types.Message):
    text = (
        "🏥 Жалобы пациента — куда и как обращаться (по нарастающей):\n"
        "1) руководству медорганизации (заведующий/администратор/главврач) — лучше письменно\n"
        "2) служба поддержки пациента/внутренний контроль (если есть)\n"
        "3) территориальный орган управления здравоохранением\n"
        "4) при необходимости — суд/правоохранительные органы\n\n"
        "Напишите: «жалоба на врача» или «куда жаловаться» — и уточните: поликлиника/стационар, гос/частная."
    )
    await message.answer(text, reply_markup=menu)


@dp.message_handler(lambda m: m.text == "✍️ Информированное согласие")
async def btn_informed_consent(message: types.Message):
    text = (
        "✍️ Информированное согласие — про документы перед вмешательством:\n"
        "• что обязаны объяснить пациенту (цель, риски, альтернативы, последствия отказа)\n"
        "• можно ли не подписывать и как задавать вопросы\n"
        "• как оформить отказ (когда применимо)\n\n"
        "Напишите: «согласие на операцию», «не дают объяснений», «хочу отказаться» — я дам типовой алгоритм."
    )
    await message.answer(text, reply_markup=menu)


@dp.message_handler(lambda m: m.text == "🔒 Врачебная тайна")
async def btn_med_secret(message: types.Message):
    text = (
        "🔒 Врачебная тайна — про конфиденциальность медданных:\n"
        "• что относится к тайне (диагноз, результаты, факт обращения и др.)\n"
        "• кому и когда можно раскрывать информацию\n"
        "• что делать при подозрении на разглашение\n\n"
        "Напишите: «разгласили диагноз», «дали справку без меня», «сообщили родственникам» — "
        "я подскажу типовой порядок действий."
    )
    await message.answer(text, reply_markup=menu)


@dp.message_handler(lambda m: m.text == "👮 Ответственность медработников")
async def btn_responsibility(message: types.Message):
    text = (
        "👮 Ответственность медработников — общий обзор:\n"
        "• дисциплинарная (внутри организации)\n"
        "• гражданско-правовая (возмещение вреда)\n"
        "• административная (КоАП) и уголовная (УК) — при наличии состава\n\n"
        "Чтобы уточнить, напишите: был ли вред здоровью, есть ли документы (выписка/назначения), "
        "и что именно вы хотите: «жалоба», «компенсация», «оценка действий врача»."
    )
    await message.answer(text, reply_markup=menu)
@dp.message_handler(lambda m: m.text in SECTION_TO_QUERY)
async def handle_section_buttons(message: types.Message):
    query = SECTION_TO_QUERY.get(message.text)
    entry = find_answer(query)

    if entry:
        answer = (entry.get("answer") or entry.get("a") or "").strip()
        if answer:
            await message.answer(answer, reply_markup=menu)
            return

    await message.answer(
        "Информация по разделу готовится. "
        "Вы можете задать уточняющий вопрос текстом.",
        reply_markup=menu
    )
@dp.message_handler()
async def handle_text(message: types.Message):
    user_text = (message.text or "").strip()

    entry = find_answer(user_text)
    if entry:
        answer = (entry.get("answer") or entry.get("a") or "").strip()
        law = entry.get("law")
        if law:
            answer += f"\n\n🔹 Нормативная база: {law}"
        answer += f"\n\n{DISCLAIMER}"
        await message.answer(answer, reply_markup=menu)
    else:
        await message.answer(
            "Не нашёл точного ответа в базе знаний.\n"
            "Попробуйте переформулировать вопрос проще (1–2 ключевых слова) "
            "или нажмите «✉️ Задать вопрос преподавателю».",
            reply_markup=menu
        )

if __name__ == "__main__":
    executor.start_polling(dp, skip_updates=True)
