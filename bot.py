...
@dp.message_handler(lambda m: m.text == "📄 Нормативная база")
...
@dp.message_handler(lambda m: m.text == "✉️ Задать вопрос преподавателю")
...
async def handle_section_buttons(message: types.Message):
    ...
    if entry:
        ...
        # Дисклеймер — только тут, один раз
        answer += f"\n\n{DISCLAIMER}"
...
@dp.message_handler(lambda m: m.text and (m.text not in SECTIONS) and (not m.text.startswith("/")))
